...
<!---START OF STEP 10 IN THE PROJECT 6 PROGRAM AND DEPLOY_V!.DOCX--->
						<!-- PART 2-->
<?php	// if post back
if (!isset($_POST["btnProcessCall"]) && !isset($_POST["btnDispatch"])) header("Location: logcall.php");
?>
<?php
if (isset($_POST["btnDispath"]))
{
	require_once 'db.php';
	...
	// RETRIEVE INCIDENT_ID FPR THE NEWLY INSERTED INCIDENT
	...
	for($i=0; $i < $numOfPatrocarDispatched; $i++)
	{
		...
		$sql = "UPDATE patrolcar SET patrolcar_status_desc_id='1' WHERE patrolcar_id = ?";
...
<table class="ContentStyle">
	...
	require_once 'db.php';
	// create database connection_abort
	$mysqli = mysqli_connect(DB_SERVER, DB_USER, DB_PASSWORD, DB_DATABASE);
	// Check connection_abort
	if ($mysqli->connect_errno) {
		die("Failed to connect to MySQL: ".$mysqli->connect_errno);
	}
